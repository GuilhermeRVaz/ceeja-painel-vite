...
class EnrollmentNotifier extends StateNotifier<EnrollmentState> {
  ...
  Future<void> uploadAndExtractData() async {
    ...
    try {
      ...
      Uri.parse('http://127.0.0.1:5000/extract-data'),
  ...
  Future<void> submitEnrollment() async {
    final enrollmentId = state.enrollmentId;
    if (enrollmentId == null) {
      throw Exception("Matrícula não iniciada. Não é possível enviar.");
    }

    state = state.copyWith(isLoading: true, errorMessage: null);

    try {
      // Prepara todos os dados confirmados para enviar em uma única requisição
      final body = {
        'enrollmentId': enrollmentId,
      ...
      // Chama a nossa nova rota unificada no backend
      final response = await http.post(
        Uri.parse('http://127.0.0.1:5000/finalize-and-process'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(body),
      );

      if (response.statusCode != 200) {
        throw Exception('Falha ao finalizar matrícula no backend. Status:  {response.statusCode}');
      }
      ...
      state = state.copyWith(isLoading: false, errorMessage: e.toString());
      print("‼️ Erro ao finalizar a matrícula: $e");
      rethrow;
    } finally {
